...
from utils.dbutil.student_db import find_and_move_old_students
...
def fetch_single_student_from_web(user_id: str, headless_input_bool: bool = False) -> dict[str:str]:
    ...
    driver.get(url=f"https://elevkonto.linkoping.se/users?FreeText={user_id}")
    ...
    for row in rows:
        ...
        if data_item_id is not None:
            ...
            try:
                ...
                raise NoUserFoundError(f"no user found with id:{id} | account_user_name:{user_id}")
            try:
                pw = driver.find_element(by=By.XPATH, value="/html/body/div[1]/div/article/div[2]/div/div[2]/div[1]/div[5]/div[2]/span").text
            except selenium.common.exceptions.NoSuchElementException as e:
                print(f" proccess failed id:{id} no pw found for user : {user_id}")
                raise NoUserFoundError(f"no user found with id:{id} | account_user_name:{user_id}")
            else:
                driver.close()
                save_student_information_to_db(user_id=user_id,
                                               ...
                                               google_pw=pw,
                                               session=s)
                return {"account_user_name": user_id,
                        "first_name": first_name,
                        "last_name": last_name,
                        "klass": klass,
                        "skola": skola,
    ...
    for filepath in old_files:
        ...
        try:
            if user_name in fetched_students.keys():
                user = fetch_single_student_from_web(user_id=user_name)
                print(user)
